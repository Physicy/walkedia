...
// et mode de carte. Les deux sont maintenant LUS par MapScreen.tsx (calque =
// ce que la géométrie raconte, mode = ce qu'il y a dessous et son habillage) ;
// ils ne faisaient avant qu'un aller-retour vers AsyncStorage, d'où une
// feuille de réglage qui enregistrait un choix sans que la carte change quoi
// que ce soit.
//
...
// dessine réellement.
//
// Le choix du mode (ancienne rangée « Fond » à deux puces) est partagé avec
// la feuille ouverte depuis la carte elle-même, voir MapModeSheet.tsx.
...
import { MapModeChoix } from './MapModeSheet';
  ...
  return (
    <Pressable style={styles.voile} onPress={onFermer}>
      <Pressable style={[styles.feuille, { paddingBottom: insets.bottom + 22 }]} onPress={(e) => e.stopPropagation()}>
        ...
        {/* Défile : la grille des modes a allongé la feuille au-delà d'un
            petit écran. */}
        <ScrollView style={styles.defilement} showsVerticalScrollIndicator={false}>
          ...
          <Eyebrow style={styles.section}>{t('settings.mapModeTitle')}</Eyebrow>
          <MapModeChoix
            mode={mapBackground}
            onChoisirMode={onChoisirFond}
            saison={mapSeason}
