...
interface WalkediaState {
  ...
  startStatus: string;
  // Séparé du texte lui-même (qui est maintenant traduit, donc son contenu
  // ne peut plus servir à deviner s'il s'agit d'une erreur — voir
  // StartScreen.tsx, qui utilisait auparavant une regex sur le français).
  startStatusError: boolean;
...
export function useWalkedia() {
  ...
  const checkBackgroundImport = useCallback(async () => {
    ...
    Alert.alert(
      translate('alert.walkDetectedTitle'),
      translate('alert.walkDetectedBody', { minutes: mins, distance: Math.round(found.distance) }),
      [
        { text: translate('alert.ignore'), style: 'cancel' },
        { text: translate('alert.importAction'), onPress: () => importShadowWalk(found.fixes, found.startedAt) },
      ]
    );
  }, [importShadowWalk, translate]);
    ...
    (lat: number, lon: number, accuracy: number | null, t: number, speed: number | null) => {
      ...
      if (speedKmh != null && speedKmh > SPEED_LIMIT_KMH) {
        ...
        if (state.speedStreak >= SPEED_STREAK_LIMIT) {
          state.speedStreak = 0;
          toast(translate('toast.sessionStoppedSpeed', { speed: Math.round(speedKmh) }), 6000);
          // eslint-disable-next-line @typescript-eslint/no-use-before-define
        ...
        for (const edgeId of state.matcher.feed(fix.lat, fix.lon, fix.accuracy)) {
          ...
          for (const jid of graph.edgeJunctions.get(edgeId) || []) {
            ...
            if (j && checkJunction(j)) {
              ...
              toast(translate('toast.junctionCompleted'));
            }
          }
          persist();
        }
  ...
  const endSession = useCallback(() => {
    ...
    toast(translate('toast.sessionEnded', { minutes: mins, edges: newEdges.size, junctions: newInter.size }), 6000);
    rerender();
  }, [state, persist, toast, rerender, translate]);
